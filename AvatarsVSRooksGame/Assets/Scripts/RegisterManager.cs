...
public class RegisterManager : MonoBehaviour
{
    ...
    [SerializeField] private GameObject buttonRegister; // asigna el GameObject del bot贸n
    ...
    private IEnumerator WaitForFirebaseInitialization()
    {
        ...
        {
            Debug.Log("Esperando inicializaci贸n de Firebase...");
            yield return null;
        }

        auth = FirebaseAuth.DefaultInstance;

        // 馃敡 Soluci贸n: inicializar la base con URL manual
    ...
    public void OnRegisterButtonPressed()
    {
        ...
        {
            statusText.text = "La contrase帽a debe tener al menos 6 caracteres.";
            return;
        }

        // bloquear bot贸n para evitar m煤ltiples clicks
        var btnComp = buttonRegister.GetComponent<UnityEngine.UI.Button>();
        if (btnComp != null) btnComp.interactable = false;

        // comprobar username 煤nico en DB, luego crear usuario
    ...
    void CreateAuthUser(string username, string email, string password)
    {
        auth.CreateUserWithEmailAndPasswordAsync(email, password).ContinueWithOnMainThread(task =>
        {
            ...
            // 2) Si es null, intentar usar reflexi贸n para sacar Result / User
            if (newUser == null)
            {
                try
                {
                ...
                {
                    Debug.LogWarning("Reflexi贸n: no se pudo extraer usuario desde task.Result: " + ex.Message);
                }
            }
    ...
    void SaveAdditionalUserData(FirebaseUser firebaseUser, string username, string email)
    {
        ...
        dbRef.Child("users").Child(uid).SetValueAsync(userData).ContinueWithOnMainThread(task =>
        {
            if (task.IsFaulted)
            {
                ...
                statusText.text = "Error guardando datos. El usuario fue creado en Auth, pero fall贸 la DB.";
                UnlockRegisterButton();
            }
            else
            {
            ...
            if (inner is FirebaseException fbEx)
            {
                ...
                switch (code)
                {
                    case AuthError.EmailAlreadyInUse:
                        msg = "El correo ya est谩 registrado.";
                        break;
                    case AuthError.InvalidEmail:
                        msg = "Correo inv谩lido.";
                        break;
                    case AuthError.WeakPassword:
                        msg = "Contrase帽a d茅bil.";
                        break;
                    default:
                        msg = fbEx.Message;
                        break;
    ...
    // Bot贸n Cancel: vuelve a la escena Login (si la tienes)
